...
class roostooAPI:
    ...
    def get_balance(self):
        ...
        headers, payload, total_params_string = self._get_signed_headers()

        try:
            # 2. Send the request
            # In a GET request, the payload is sent as 'params'
            response = requests.get(self.BALANCE_URL, headers=headers, params=payload)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def get_pending_count(self):
        ...
        try:
            response = requests.get(self.PENDING_URL, headers=headers, params=payload)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def query_order(self, order_id=None, pair=None, pending_only=None):
        ...
        headers, final_payload, total_params_string = self._get_signed_headers(payload)
        headers['Content-Type'] = 'application/x-www-form-urlencoded'

        try:
            response = requests.post(self.QUERY_ORDER_URL, headers=headers, data=total_params_string)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def cancel_order(self, order_id=None, pair=None):
        ...
        try:
            response = requests.post(self.CANCEL_ORDER_URL, headers=headers, data=total_params_string)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
